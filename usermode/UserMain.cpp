...
int main() {
    ...
    for (; !GetAsyncKeyState(VK_F12); Sleep(500)) {
        ...
        //0x9
        if (POINTER(base_addr_t, AION::GAMEDLL::target::pointer_1, _addr_t, 4, ptr_cache, 0, true) == false) {
			continue;
		}

        if (POINTER(_addr_t, AION::GAMEDLL::target::pos::pointer_2_pos, _addr_t, 4, ptr_cache, 1, false) == false) {
			continue;
		}
        if (driver->ReadMemType(_addr_t + AION::GAMEDLL::target::pos::z_cord, _z_pos) != STATUS_SUCCESS ||
            driver->ReadMemType(_addr_t + AION::GAMEDLL::target::pos::y_cord, _y_pos) != STATUS_SUCCESS ||
            driver->ReadMemType(_addr_t + AION::GAMEDLL::target::pos::x_cord, _x_pos) != STATUS_SUCCESS ||
