...
#define MakePtr(Type, Base, Offset) ((Type)(ULONG_PTR(Base) + (ULONG_PTR)(Offset)))
...
)
{
	...
	*/
	...
	/*print("MmUnloadedDrivers ModuleEnd: %x", MmUnloadedDrivers->ModuleEnd);
	print("MmUnloadedDrivers ModuleStart: %x", MmUnloadedDrivers->ModuleStart);
	print("MmUnloadedDrivers Name: %s", MmUnloadedDrivers->Name);
	print("MmUnloadedDrivers UnloadTime: %x", MmUnloadedDrivers->UnloadTime);*/

	print("MmUnloadedDrivers Addr: %x", MmUnloadedDrivers);
	print("MmLastUnloadedDriver Addr: %x", MmLastUnloadedDriver);
	return STATUS_SUCCESS;
}
...
namespace clear {
	void clearCache(UNICODE_STRING DriverName, ULONG timeDateStamp) {
		...
		if (!LocatePiDDB(&PiDDBLock, &PiDDBCacheTable)) {
			print("ClearCache Failed");
			return;
		}

		print("Found PiDDBLock and PiDDBCacheTable");
		print("Found PiDDBLock %x", PiDDBLock);
		print("Found PiDDBCacheTable %x", PiDDBCacheTable);
		...
		if (pFoundEntry == nullptr)
		{
			...
			print("ClearCache Failed (Not found)");
			return;
		}
